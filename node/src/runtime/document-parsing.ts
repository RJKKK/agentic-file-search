...
function defaultBridgeScript(repositoryRoot: string): string {
  return resolve(repositoryRoot, "node", "python", "document_parsing_bridge.py");
}
...
export class PythonDocumentParserBridge implements PythonDocumentParserExecutor {
  ...
  async parseDocument(filePath: string, selector?: ParseSelector | null): Promise<ParsedDocument> {
      ...
      child.on("error", (error) => {
        rejectPromise(
          new DocumentParseError(
            ...
            `Failed to start Python bridge: ${String(error)}`,
          ),
        );
      });
      child.on("close", (code) => {
        if (code !== 0) {
          rejectPromise(
            new DocumentParseError(
              resolve(filePath),
              "python_bridge_failed",
              stderr.trim() || `Python bridge exited with code ${code}.`,
            ),
          );
          return;
        }
    ...
    let parsedResponse;
    try {
      parsedResponse = PythonBridgeResponseSchema.parse(JSON.parse(raw));
    } catch (error) {
      throw new DocumentParseError(
        resolve(filePath),
