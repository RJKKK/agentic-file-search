...
async function hydrateSessionSnapshot(sessionId) {
  ...
  try {
    ...
    // Best-effort sync for sessions that finish before the SSE handlers render.
  }
}
...
function traceDetails(event) {
  ...
  switch (event.type) {
    ...
    case "ask_human":
      ...
      break;
    case "answer_started":
      add("草稿", shortText(data.draft_final_result, 320));
      break;
    case "answer_delta":
      add("增量", data.delta_text);
      break;
    case "answer_done":
      add("完成", shortText(data.final_result, 600));
      break;
    case "complete":
