...
function promptForStart(input: {
  ...
  const scopeName = input.collectionName ? `collection '${input.collectionName}'` : "selected documents";
...
export class ExplorationWorkflowService {
  ...
  private async createRuntime(session: ExploreSession): Promise<ExplorationRuntimeState> {
    ...
    const indexSearch = new IndexSearchService({
      ...
      scopeLabel: scope.collection?.name ?? `${scope.documentIds.length} selected documents`,
      emitRuntimeEvent,
    });
